...
class Capybara::Driver::Facebook < Capybara::Driver::Base
  ...
  def initialize(app)
    @app = app
    @default_request_params = {:fb_sig_in_canvas => '1'}
  end

  def visit(path, attributes = {})
    return if path.gsub(/^#{current_path}/, '') =~ /^#/
    get(path, @default_request_params.merge(attributes), env)
    follow_redirects!
    cache_body
  end
  ...
  def merge_params!(params={})
    @default_request_params.merge!(params)
  end
